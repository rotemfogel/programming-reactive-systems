...
class Replica(val arbiter: ActorRef, persistenceProps: Props) extends Actor with ActorLogging {
  ...
  val kv = mutable.Map.empty[String, (String, Long)]
  ...
  val _replicatorId = new AtomicLong(0)

  def nextReplicatorId(): Long = {
    _replicatorId.incrementAndGet()
  }

  def replicate(op: Operation,
                replicators: Set[ActorRef],
                currentReplicationAcks: Map[Long, (String, Option[ActorRef], Int)]): Map[Long, (String, Option[ActorRef], Int)] = {
    log.debug("replicators {}", replicators)
    ...
    replicators foreach { x =>
      log.debug("replicate {} on {}", replicationMsg, x)
      x ! replicationMsg
    }
    if (replicators.nonEmpty) {
      currentReplicationAcks + ((op.id, (op.key, Some(sender), replicators.size)))
    } else {
